...
function MarkdownBlock({ text }: { text: string }) {
  ...
  lines.forEach((line, i) => {
    ...
    if (trimmed.startsWith('### ')) {
      ...
      elements.push(<h3 key={i} className="mt-4 mb-1 text-base font-bold text-[#003366]">{trimmed.slice(4)}</h3>)
    } else if (trimmed.startsWith('## ')) {
      flushList(`list-${listStartIdx}-${i}`)
      elements.push(<h2 key={i} className="mt-5 mb-1 text-lg font-bold text-[#003366] border-b border-slate-200 pb-1">{trimmed.slice(3)}</h2>)
    } else if (trimmed.startsWith('# ')) {
      flushList(`list-${listStartIdx}-${i}`)
      elements.push(<h1 key={i} className="mt-5 mb-1 text-xl font-bold text-[#003366]">{trimmed.slice(2)}</h1>)
...
export function EssayGraderScreen() {
  ...
  return (
    <div className="space-y-8">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[#003366]">Essay Grader</h2>
          <p className="mt-1 text-slate-600">Submit your essay for a full judge-rubric evaluation.</p>
        </div>
        <button
          type="button"
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm md:p-6">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-500">Essay Topic</h3>
